    ...
    def build(self):
        ...
        for i in segments:
            ...
            if self.config.use_surrogate:
                ...
                obj.enth_temp_surrogate = Constraint(
                    expr=obj.temperature_fluid == smooth_piecewise_linear(
                        self.config.surrogate_data,
                        obj.outlet.enth_mol[0])
                )

                obj.tube_heat_transfer_eq = Constraint(
                    expr=obj.heat_duty[0] == obj.htc
                         * (Constants.pi * obj.d_tube_outer * obj.segment_length)
                         * (obj.temperature_wall - obj.temperature_fluid),
    ...
    def build(self):
        ...
        def period(m):
            ...
            if operating_mode == "charge" or operating_mode == "combined":
                charge_data = get_HT_surrogate_data(
                    self.config.inlet_pressure_data["charge"],
            ...
            if operating_mode == "discharge" or operating_mode == "combined":
                discharge_data = get_HT_surrogate_data(
                    self.config.inlet_pressure_data["charge"],
                    generate_plots=False)
